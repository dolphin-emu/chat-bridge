{
  description = "chat-bridge, Dolphin's IRC-Discord bridge bot";
  ...
  let
    perSystem = flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        venv = pythonSet.mkVirtualEnv "chat-bridge-env" workspace.deps.default;
      in {
        packages.chat-bridge = venv;
        packages.default = venv;

        apps.default = {
          type = "app";
          program = "${venv}/bin/chat-bridge";
        };
  ...
  in
    perSystem // {
      overlays.default = final: prev: {
        chat-bridge = self.packages.${final.system}.chat-bridge;
      };
